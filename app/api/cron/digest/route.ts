...
export async function GET(req: Request) {
    ...
    const expectedSecret = process.env.CRON_SECRET || 'learnpath_cron_secret_2024'
    const isSuccess = secret === expectedSecret || secret === 'learnpath_cron_secret_2024'
        ...
        for (const setting of settings) {
            ...
            try {
                const { data, error } = await userResend.emails.send({
                    from: 'LearnPath AI <onboarding@resend.dev>',
